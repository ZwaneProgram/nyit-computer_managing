...
**Next, in order:**
1. **Bundles** — wire `BundlesView` to real API. Backend has `bundles` + `bundle_items` tables but **no bundle routes yet** — build `routes/bundles.ts` (CRUD; auto price/cost from component products; the `bundles` table has `discount_pct` not a stored price, so compute). Then rewrite `BundlesView` off mock onto the data layer (extend `src/data/inventory.ts` or add `src/data/bundles.ts`).
2. Then Sales (atomic checkout flips serials to sold) → Dashboard/Analytics (real SQL aggregations) → Settings/cleanup.
3. Once Bundles/Sales/Dashboard/Analytics are wired, **delete mock `src/data/catalog.ts`** and the Sidebar low-count's dependency on it.
