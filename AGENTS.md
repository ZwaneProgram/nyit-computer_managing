...
**Next, in order:**
1. **Finish Inventory:** (a) **Categories management** page/section (NEW, CRUD — backend ready at `/api/categories`); (b) **Edit product** (reuse AddProduct form in edit mode → `updateProduct` PUT; currently only create+delete+serial-add/remove exist); (c) optional: CSV export (was removed from the toolbar).
